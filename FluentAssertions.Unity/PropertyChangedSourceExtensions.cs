...
namespace FluentAssertions
{
    ...
    public static class PropertyChangedSourceExtensions
    {
        ...
        /// </summary>
        public static IEventRecorder WithArgs<T>(this IEventRecorder eventRecorder, Func<T, bool> predicate)
            where T : EventArgs
        {
            ...
            if (!eventRecorder.Any(@event => predicate(@event.Parameters.OfType<T>().Single())))
            {
                Execute.Assertion
                    .FailWith("Expected at least one event with arguments matching supplied predicate, but found none.");
            }
